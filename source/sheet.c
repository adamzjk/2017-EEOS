...
/*void sheet_refreshmap_old(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
	...
	struct SHEET *sht;
	/* refresh”ÍˆÍ‚ª‰æ–ÊŠO‚É‚Í‚Ýo‚µ‚Ä‚¢‚½‚ç•â³ */
	if (vx0 < 0) { vx0 = 0; }
	if (vy0 < 0) { vy0 = 0; }
	if (vx1 > ctl->xsize) { vx1 = ctl->xsize; }
	if (vy1 > ctl->ysize) { vy1 = ctl->ysize; }
	for (h = h0; h <= h1; h++) {
		...
		buf = sht->buf;
		sid = sht - ctl->sheets0;
		/* vx0`vy1‚ðŽg‚Á‚ÄAbx0`by1‚ð‹tŽZ‚·‚é */
		bx0 = vx0 - sht->vx0;
		by0 = vy0 - sht->vy0;
		bx1 = vx1 - sht->vx0;
		by1 = vy1 - sht->vy0;
		...
		if (binfo->vmode == 8) {
			...
			if ((sht->vx0 & 2) == 0) {
				/* 4ƒoƒCƒgŒ^ */
				i  = (bx0 + 1) / 2; /* bx0‚ð2‚ÅŠ„‚Á‚½‚à‚Ìi’[”Ø‚èã‚°j */
				i1 =  bx1      / 2; /* bx1‚ð2‚ÅŠ„‚Á‚½‚à‚Ìi’[”Ø‚èŽÌ‚Äj */
				i1 = i1 - i;
				sid2 = sid | sid << 8;
				for (by = by0; by < by1; by++) {
					vy = sht->vy0 + by;
					for (bx = bx0; bx < bx1 && (bx & 1) != 0; bx++) {	/* ‘O‚Ì’[”‚ð1ƒoƒCƒg‚¸‚Â */
						vx = sht->vx0 + bx;
						if (map[vy * ctl->xsize + vx] == sid) {
					...
					for (bx += i1 * 2; bx < bx1; bx++) {				/* Œã‚Ì’[”‚ð1ƒoƒCƒg‚¸‚Â */
						vx = sht->vx0 + bx;
						if (map[vy * ctl->xsize + vx] == sid) {
							*(unsigned short *)	&vram[(vy * ctl->xsize + vx) * 2] =
								buf[by * sht->bxsize + bx];
...
/*void sheet_refreshsub_old(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
	int h, bx, by, vx, vy, bx0, by0, bx1, by1;
