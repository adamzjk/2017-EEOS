...
VBEMODE	EQU		0x117			; 1024 x  768 x 16bit 彩色
; 显示模式
...
;	0x111 :  640 x  480 x 16bit 彩色
;	0x114 :  800 x  600 x 16bit 彩色
;	0x117 : 1024 x  768 x 16bit 彩色
;	0x11A : 1280 x 1024 x 16bit 彩色
...
; 画面模式信息的确认
		CMP		BYTE [ES:DI+0x19],16		;颜色数
		JNE		scrn320
		CMP		BYTE [ES:DI+0x1b],6			;颜色的指定方法(调色板模式)
		JNE		scrn320
...
;	画面设置
		...
		MOV		AX,0x4f02
		INT		0x10
		MOV		BYTE [VMODE],16	; 屏幕的模式（参考C语言的引用）
		MOV		AX,[ES:DI+0x12]
		MOV		[SCRNX],AX
		MOV		AX,[ES:DI+0x14]
		MOV		[SCRNY],AX
