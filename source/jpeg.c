...
int info_JPEG(struct DLL_STRPICENV *env,int *info, int size, unsigned char *fp0)
{
...
int decode0_JPEG(struct DLL_STRPICENV *env,int size, unsigned char *fp0, int b_type, unsigned char *buf, int skip)
{
	JPEG *jpeg = (JPEG *) (((int *) env) + 128);
	jpeg->fp = fp0;
	jpeg->fp1 = fp0 + size;
